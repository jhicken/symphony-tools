...
function renderLogarithmicScaleInCharts(shouldToggle = true) {
...
function updateLButtonActiveState() {
...
function addLogarithmicToggleToCharts() {
  window.Highcharts.charts.forEach(chart => {
    if (chart) {
      const container = chart.container;
      const existingLButton = container.querySelector(`.${buttonClassName}`);
      if (existingLButton) {
        return;
      }

      const lButton = document.createElement('button');
      lButton.className = buttonClassName;
      lButton.innerText = 'L';
      lButton.title = 'Toggle logarithmic scale (Alt + L)';

      lButton.addEventListener('click', () => {
        if (highchartsConfigured) {
          renderLogarithmicScaleInCharts(true);
          updateLButtonActiveState();
        }
      });
...
function watchForAndConfigureHighcharts() {
  ...
  if (window.Highcharts) {
    ...
    if (window.Highcharts.charts.length !== chartsCount) {
      ...
      renderLogarithmicScaleInCharts(false);
      updateLButtonActiveState();
    }
  } else {
